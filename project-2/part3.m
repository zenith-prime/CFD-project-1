...
for j = 2:nx
    ...
    while (true)
        u_old = u(:,j);
        for i = 2:ny-1
            u(i,j) = (u(i, j-1)^2/dx - v(i,j-1)*(u(i+1, j) - u(i-1,j) ...
                + u(i+1, j-1) - u(i-1, j-1))/(4*dy) + (u(i+1, j) + ...
                u(i-1,j) + u(i+1, j-1) - 2*u(i, j-1) + ...
                u(i-1, j-1))*nu/(2*dy^2))/(u(i, j-1)/dx+ nu/dy^2);
        end
        residual = sqrt(sum((u(:,j) - u_old).^2));
        p = p + 1;
        if(residual<tol || p>=max_iter)
            break;
        end
    end
